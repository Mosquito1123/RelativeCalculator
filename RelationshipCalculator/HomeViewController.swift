...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 13.0, *) {
            navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(systemName: "star.fill"), style: .plain, target: self, action: #selector(switchTheme))
        } else {
            // Fallback on earlier versions
            navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(named: "night"), style: .plain, target: self, action: #selector(switchTheme))
